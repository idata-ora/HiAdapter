...
from utils.cModule import StainNet
from utils.cfusion import cDeepFusion

from .layers import Mlp, PatchEmbed, DropPath, PatchDropout, resample_abs_pos_embed 
...
def vit_large_patch16_224(patch_size=16, pretrained=False, args=None, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model = VisionTransformer(patch_size=patch_size, embed_dim=1024, depth=24, num_heads=16, args=args, **kwargs)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        ##todo
        if self.args.MA:
            self.num_fusion_layers = len(self.args.deep_fusion_layers)

            self.scale_factor = 32
            self.prompt_generator = PromptGenerator(self.scale_factor, self.embed_dim,
                                                    self.num_fusion_layers, # self.depth,
                                                    img_size, patch_size)
        ...
        if self.args.SI:
            self.MulLayer = StainNet(embed_dim=embed_dim, img_size=img_size)
            self.cfusion_layer = cDeepFusion(embed_dim=embed_dim)
    ...
    def forward_features(self, inx, MA):

        x = self.patch_embed(inx)


        if self.args.MA:
            handcrafted_feature = []
            for i in range(len(MA)):
                handcrafted_feature.append(self.prompt_generator.init_handcrafted(MA[i]))
            prompt = self.prompt_generator.get_prompt(handcrafted_feature)
        ...
        if self.args.SI and self.args.cdeep_fusion:
            x_c = self.MulLayer(inx)
            x = self.cfusion_layer(x, x_c)
        ...
        if self.args.MA:
            ...
            j=0
            for i, blk in enumerate(self.blocks):
                if i in self.args.deep_fusion_layers:
                    x[:, 1:, :] = prompt[j] + x[:, 1:, :]
                    j=j+1
                x = blk(x)
    ...
    def forward(self, x, MA=None):
        features= self.forward_features(x, MA)
...
class PromptGenerator(nn.Module):
    def __init__(self, scale_factor, embed_dim, depth, img_size, patch_size):
        """
        Args:
        """
        ...
        for i in range(self.depth):
            lightweight_mlp0 = nn.Sequential(
                nn.Linear(self.embed_dim//self.scale_factor, self.embed_dim//self.scale_factor),
                nn.GELU()
            )
            setattr(self, 'lightweight_mlp0_{}'.format(str(i)), lightweight_mlp0)
            lightweight_mlp1 = nn.Sequential(
                nn.Linear(self.embed_dim//self.scale_factor, self.embed_dim//self.scale_factor),
                nn.GELU()
            )
            setattr(self, 'lightweight_mlp1_{}'.format(str(i)), lightweight_mlp1)
            lightweight_mlp2 = nn.Sequential(
                nn.Linear(self.embed_dim//self.scale_factor, self.embed_dim//self.scale_factor),
                nn.GELU()
            )
            setattr(self, 'lightweight_mlp2_{}'.format(str(i)), lightweight_mlp2)
    ...
    def get_prompt(self, handcrafted_feature):
        
        prompts = []
        for d in range(self.depth):
            tmp = [x.clone() for x in handcrafted_feature] 
            for i in range(len(handcrafted_feature)):
                N, C, H, W = tmp[i].shape
                tmp[i] = tmp[i].view(N, C, H*W).permute(0, 2, 1)
                lightweight_mlp = getattr(self, 'lightweight_mlp{}_{}'.format(str(i), str(d)))
                # prompt = proj_prompt(prompt)
                if i == 0:
                    prompt = lightweight_mlp(tmp[i])
                else:
                    prompt = prompt + lightweight_mlp(tmp[i])
